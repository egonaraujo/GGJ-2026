...
@export var active_position: Vector2
@export var active_scale: Vector2

@export var Branches: float = 0
@export var Fertilizer: float = 0
@export var Water: float = 0
@export var Bugspray: float = 0
@export var healthy_fertilizer: float = 10
@export var healthy_branches: float = 3
@export var healthy_water: float = 1.6
@export var healthy_kills: float = 1.6
...
func _ready():
	...
	if(healthy_water == 0):
		$Dried.modulate.a = 0
	for i in range(10):
		var branchAsset = $Leaves.find_child("Branch%d"%[i])
		if(!branchAsset):
			break
		...
		if(healthy_branches <= i):
			if(branchAsset):
				var branchCol = $Leaves/Colliders.find_child("Colliders%dCollider"% [i])
