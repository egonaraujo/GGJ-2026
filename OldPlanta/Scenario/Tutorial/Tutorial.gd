...
func _ready():
	...
	workstation.connect("plant_healthy", Callable(self, "_plantHealthy"))
	workstation.set_process_input(false)
	workstation.setActivePlant(plant)
	workstation.get_node("Shears").connect("tool_selected", Callable(self, "selectTool"))
	workstation.get_node("Watercan").connect("tool_selected", Callable(self, "selectTool"))
	workstation.get_node("BugSpray").connect("tool_selected", Callable(self, "selectTool"))
	workstation.get_node("Fertilizer").connect("tool_selected", Callable(self, "selectTool"))


func _plantHealthy(_plantname):
	plantHealthy= true
	plantTimer=0
...
func _process(delta):
	...
	if index == 4:
		...
		if per_cent > 0.8:
			...
			$"hand-upside".set_scale(Vector2(1.8 - per_cent, 1.8 - per_cent))
		if anim_time > anim_end:
			$"hand-click".hide()
			anim_time = 0
...
func _on_Skip_pressed():
	get_tree().change_scene_to_file("res://Scenario/Phases/Phase01/Phase01.tscn")
...
func _setSceneFromIndex():
	...
	if index%2==0:
		$TextSkip.add_theme_color_override("default_color", Color("ff877b"))
		$ProximoText.add_theme_color_override("default_color", Color("ff877b"))
		$TextBack.add_theme_color_override("default_color", Color("ff877b"))
	else:
		$TextSkip.add_theme_color_override("default_color", Color("305300"))
		$ProximoText.add_theme_color_override("default_color", Color("305300"))
		$TextBack.add_theme_color_override("default_color", Color("305300"))
