...
@export var SlashSound: AudioStreamOggVorbis
@export var SpraySound: AudioStreamOggVorbis
@export var WaterSound: AudioStreamOggVorbis
@export var FertilizerSound: AudioStreamOggVorbis
@export var trailLength: float
...
func _ready():
	...
	_activePlant = null
	_oldPlant = null
	$Fertilizer.connect("tool_selected", Callable(self, "selectTool"))
	$Shears.connect("tool_selected", Callable(self, "selectTool"))
	$Watercan.connect("tool_selected", Callable(self, "selectTool"))
	$BugSpray.connect("tool_selected", Callable(self, "selectTool"))
...
func _process(delta):
	...
	for particle in particlesToRemove:
		...
		(particle as GPUParticles2D).free()
...
func _input(event):
	...
	if( event is InputEventScreenDrag):
		...
		if(!stillOnPlant && isHolding):
			...
			holdPlant()
		if(slashedPlant && event.screen_velocity.length() > 200):
...
func updateEffectAndSound(event, particles : GPUParticles2D ,sound):
	if( event is InputEventScreenDrag):
		if(particles != null):
...
func setActivePlant(plant:Node2D):
	...
	plantTimer = 0
	_activePlant.connect("plant_watered", Callable(self, "plant_watered"))
	_activePlant.connect("plant_slashed", Callable(self, "plant_slashed"))
	_activePlant.connect("plant_flowered", Callable(self, "plant_flowered"))
	_activePlant.connect("plant_sprayed", Callable(self, "plant_sprayed"))
...
func _easeInOut(percent:float) -> float:
	if (percent < 0.5):
		return 4 * pow(percent, 3)
	else:
		return 1 - pow(-2 * percent + 2, 3) / 2

func _setPlantPositionPercent(plant:Node2D, percent, start, end):
	plant.position = start + (percent*(end-start))

func _setPlantScalePercent(plant:Node2D, percent, start, end):
